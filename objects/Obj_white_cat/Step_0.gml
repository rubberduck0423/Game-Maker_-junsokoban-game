    ...
    if (dx32 != 0 || dy32 != 0)
    {
        ...
        else if (b1 != noone)
        {
            
            {
                ...
                // === b1 앞 "한 줄" 먼저 스캔해서 막힘/두번째 박스(b2) 판정 ===
				var blocked = false;
				var b2 = noone;
				...
				if (dx32 != 0) {
					...
					for (var j=0; j<h1; j++) {
						...
						if (hitBox != noone && hitBox != b1) { b2 = hitBox; break; }
					}
				} else {
				...
				var pushers = 1;
				if (helper != noone && helper != id && !helper.moving) {
					helper.dir = dir; // 자동으로 방향 맞춰줌
					pushers += 1;
				}
...

        {
            ...
            if (b1c != noone)
            {
                ...
                {
                    // === b1c 앞 "한 줄" 먼저 스캔해서 막힘/두번째 박스(b2c) 판정 ===
var blocked2 = false;
var b2c = noone;
